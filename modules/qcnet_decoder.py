...
class QCNetDecoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.dataset == 'argoverse_v2':
            ...
            # TODO: Diffusion
            pos_pl = data['online_map_polygon']['position'][:, :self.input_dim]
            orient_pl = data['online_map_polygon']['orientation']
            ...
            edge_index_pl2m = radius(
                ...
                r=self.pl2m_radius,
                batch_x=data['agent']['batch'] if isinstance(data, Batch) else None,
                batch_y=data['online_map_polygon']['batch'] if isinstance(data, Batch) else None,
                max_num_neighbors=300)
        ...
        if self.dataset == 'argoverse_v2':
            ...
            edge_index_pl2m = torch.cat([edge_index_pl2m + i * edge_index_pl2m.new_tensor(
                [[data['online_map_polygon']['num_nodes']], [data['agent']['num_nodes']]]) for i in range(self.num_modes)], dim=1)
        ...
        return {
            ...
            'pi': pi,
            'r_pt2pl_diffusion_original_samples': scene_enc['r_pt2pl_diffusion_original_samples'],
            'r_pt2pl_diffusion_local_cond': scene_enc['r_pt2pl_diffusion_local_cond'],
            'r_pl2pl_diffusion_original_samples': scene_enc['r_pl2pl_diffusion_original_samples'],
            'r_pl2pl_diffusion_local_cond': scene_enc['r_pl2pl_diffusion_local_cond'],
            'cart_diffusion_original_samples': scene_enc['cart_diffusion_original_samples'],
            'cart_diffusion_local_cond': scene_enc['cart_diffusion_local_cond']
        }
