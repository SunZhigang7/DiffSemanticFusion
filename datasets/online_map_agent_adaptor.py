...
def main(args):
    ...
    for batch in tqdm(dataloader):
        ...
        if ego_hist_len < 20 or ego_fut_len < 30:
            ...
            agent_hist = batch.agent_hist.squeeze(0)[1:, -20:, :]   # #agents, time_len, (x, y, vx, vy, heading)
            agent_fut = batch.agent_fut.squeeze(0)[1:, :30, :]    # #agents, time_len, (x,y)
            agent_type = batch.agent_type[:, 1:].ravel()
            agent_name = batch.agent_names[0][1:]
            ...
            agent_name = [agent_name[i] for i in valid_agent_idx]
            assert len(agent_name) == len(agent_type)
            ...
            ego_hist = batch.agent_hist.squeeze(0)[0, -20:, :]
            ego_fut = batch.agent_fut.squeeze(0)[0, :30, :]
